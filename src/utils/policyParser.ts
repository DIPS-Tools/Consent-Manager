 ...
 */
export function parseConstraints(constraints: ODRLPermission['odrl:constraint']): Array<{
  leftOperand: string;
  operator: string;
  rightOperand: any;
  description: string;
  ...
  return constraints.map(constraint => {
    ...
    let description = `${extractReadableName(leftOperand)} ${extractReadableName(operator)}`;
    
    if (Array.isArray(rightOperand)) {
      const values = rightOperand.map(r => {
        if (typeof r === 'object' && r['@id']) return extractReadableName(r['@id']);
        if (typeof r === 'object' && r['@value']) return r['@value'];
        return String(r);
      });
      description += ` ${values.join(', ')}`;
    } else if (typeof rightOperand === 'object') {
      if (rightOperand['@id']) description += ` ${extractReadableName(rightOperand['@id'])}`;
      else if (rightOperand['@value']) description += ` ${rightOperand['@value']}`;
      else description += ` ${JSON.stringify(rightOperand)}`;
    } else {
 ...
 */
export function parseAssignees(assignee: ODRLPermission['odrl:assignee']): Array<{
  source: string;
  refinements?: Array<{
    leftOperand: string;
    operator: string;
...
export function parseODRLPolicy(policy: ODRLPolicy | null): PolicyPermission[] {
  ...
  return policy['odrl:permission'].map((permission) => {
    ...
    const purposeConstraints = constraints.filter(c => 
    ...
    if (purposeConstraints.length > 0) {
      const purposeValues = purposeConstraints.map(c => {
        if (Array.isArray(c.rightOperand)) {
          return c.rightOperand.map(p => {
            if (typeof p === 'object' && p['@id']) return extractReadableName(p['@id']);
            return String(p);
          }).join(', ');
        }
        return String(c.rightOperand);
      }).join(', ');
