...
router.get('/users', async (req, res) => {
  try {
    ...
    const response = await fetch('https://dips.soton.ac.uk/negotiation-api/users_list', {
      headers: {
...
async function getMongoUserIdFromFirebase(firebaseUid: string): Promise<string | null> {
  ...
  try {
    ...
    const requesterDoc = await db.collection('requesters').doc(firebaseUid).get();
    ...
    console.log(`❌ Firebase UID ${firebaseUid} not found in either owners or requesters collections`);
    return null;
  } catch (error) {
    console.error(`❌ Error fetching MongoDB user ID for ${firebaseUid}:`, error);
    return null;
  }
}

// Transform consent request to negotiation format
function transformConsentToNegotiation(requestData: RequestData, consumerId: string, providerId: string) {
  ...
  const odrlPermissions = permissions.map(perm => {
    const permission: any = {
      action: perm.actionRefinements?.[0]?.value || 'http://www.w3.org/ns/odrl/2/use',
      target: perm.dataset,
    };
  ...
  if (requestData.policy?.['odrl:permission'] || requestData.policy?.permission || requestData.policy?.odrl) {
    ...
    odrlPolicy = {
      ...
      ...(odrlPermissions.length > 0 && !existingPolicy['odrl:permission'] && !existingPolicy.permission ? {
  ...
  const sanitizedTitle = requestData.requestName.toLowerCase()
  ...
  if (requestData.policy?.['odrl:permission']) {
    ...
    requestData.policy['odrl:permission'].forEach((perm: any) => {
      const action = perm['odrl:action']?.['rdf:value']?.['@id'] || 
                    perm['odrl:action']?.['@id'] ||
                    perm.action;
      if (action && typeof action === 'string') {
        const actionName = action.split(/[:#\/]/).pop()?.replace(/_/g, ' ');
    ...
    for (const perm of requestData.policy['odrl:permission']) {
      if (perm['odrl:constraint']) {
        for (const constraint of perm['odrl:constraint']) {
          ...
          if (leftOp && (leftOp.includes('location') || leftOp.includes('geographic') || leftOp.includes('region'))) {
...
router.post('/negotiation/create-with-initial', async (req, res) => {
  try {
    ...
    // Transform consent request to negotiation format
    const negotiationRequest = transformConsentToNegotiation(requestData, consumerId, providerId);


    // Send to negotiation API
    const response = await fetch('https://dips.soton.ac.uk/negotiation-api/negotiation/create-with-initial', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
    ...
    if (!response.ok) {
      ...
      throw new Error(`Negotiation API error: ${response.status} - ${errorText}`);
    }
...
router.post('/negotiation/create-accepted', async (req, res) => {
  ...
  try {
    ...
    // Transform consent request to negotiation format
    console.log('🔍 STEP 3: Transforming consent request to negotiation format...');
    const negotiationRequest = transformConsentToNegotiation(requestData, mongoConsumerId, mongoProviderId);
    ...
    console.log('✅ TRANSFORMATION SUCCESS: Negotiation request created:', {
      ...
      hasOdrlPolicy: !!finalNegotiationRequest.initial_request?.odrl_policy?.odrl
    });

    // Log the complete request body (but mask sensitive data)
    console.log('📤 STEP 4: Preparing API request...');
    console.log('🔑 Token being used:', token ? `${token.substring(0, 20)}...${token.slice(-10)}` : 'No token provided');
    console.log('🌐 API endpoint: https://dips.soton.ac.uk/negotiation-api/negotiation/create-with-initial');
    console.log('📋 Request payload size:', JSON.stringify(finalNegotiationRequest).length, 'bytes');
    ...
    console.log('✅ EXTERNAL API SUCCESS:', {
      responseBody: result,
      negotiationId: result.id || result.negotiation_id || 'NOT_FOUND_IN_RESPONSE',
    ...
    if (!negotiationId) {
      console.log('⚠️  WARNING: No negotiation ID found in API response, but proceeding with update');
    } else {
    }
    ...
    console.log('✅ FIREBASE UPDATE SUCCESS: Request document updated with negotiation info');
